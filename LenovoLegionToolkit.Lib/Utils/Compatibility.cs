...
public static partial class Compatibility
{
    ...
    public static async Task<MachineInformation> GetMachineInformationAsync()
    {
        ...
        var machineInformation = new MachineInformation
        {
            ...
            Properties = new()
            {
                ...
                SupportsGodModeV3 = GetSupportsGodModeV3(supportedPowerModes, smartFanVersion, legionZoneVersion, machineType, model, biosVersion),
                SupportsGen9Features = GetSupportsGen9Features(machineType, model, biosVersion),
    ...
    /// <summary>
    /// Check if device supports Gen 9 enhanced features (Legion Slim 7i Gen 9 16IRX9)
    /// </summary>
    private static bool GetSupportsGen9Features(string machineType, string model, BiosVersion? biosVersion)
    {
        // Check for Gen 9 machine type
        if (machineType == "83G0")
            return true;

        // Check for Gen 9 model patterns
        if (model.Contains("16IRX9", StringComparison.InvariantCultureIgnoreCase))
            return true;

        // Check for specific Gen 9 BIOS versions
        var gen9BiosVersions = new BiosVersion[]
        {
            new("K1CN", null), // Gen 9 BIOS prefix
            new("K2CN", null),
            new("K3CN", null)
        };

        return gen9BiosVersions.Any(bv => biosVersion?.IsHigherOrEqualThan(bv) ?? false);
    }

    /// <summary>
    /// Check if device supports God Mode V3 (Gen 9 with enhanced EC control)
    /// </summary>
    private static bool GetSupportsGodModeV3(IEnumerable<PowerModeState> supportedPowerModes, int smartFanVersion, int legionZoneVersion, string machineType, string model, BiosVersion? biosVersion)
    {
        if (!supportedPowerModes.Contains(PowerModeState.GodMode))
            return false;

        // Gen 9 specific detection
        return GetSupportsGen9Features(machineType, model, biosVersion) && (smartFanVersion >= 8 || legionZoneVersion >= 5);
    }
