...
/// <summary>
/// God Mode Controller V3 for Legion Slim 7i Gen 9 (16IRX9)
/// Implements advanced Gen 9 features including AI thermal optimization
/// </summary>
...
{
    private readonly Gen9ECController _gen9EcController = gen9EcController;
    ...
    public override async Task ApplyStateAsync()
    {
        ...
        if (Log.Instance.IsTraceEnabled)
            Log.Instance.Trace($"Applying Gen 9 state...");

        // Apply Gen 9 hardware fixes first
        await _gen9EcController.ApplyAllGen9FixesAsync().ConfigureAwait(false);

        var (presetId, preset) = await GetActivePresetAsync().ConfigureAwait(false);

        // Apply enhanced Gen 9 power management
        if (preset.CPULongTermPowerLimit.HasValue && preset.CPUShortTermPowerLimit.HasValue)
        {
            var pl1 = preset.CPULongTermPowerLimit.Value.Value;
            var pl2 = preset.CPUShortTermPowerLimit.Value.Value;
            var gpuTgp = preset.GPUConfigurableTGP?.Value ?? 115;

            await _gen9EcController.SetPowerLimitsAsync(pl1, pl2, gpuTgp).ConfigureAwait(false);

            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Applied Gen 9 power limits - PL1: {pl1}W, PL2: {pl2}W, GPU TGP: {gpuTgp}W");
        }

        // Apply traditional WMI settings for compatibility
        ...
        if (Log.Instance.IsTraceEnabled)
            Log.Instance.Trace($"Gen 9 state applied. [name={preset.Name}, id={presetId}]");
    }

    /// <summary>
    /// Enhanced minimum fan table for Gen 9 dual fan system
    /// </summary>
    public override Task<FanTable> GetMinimumFanTableAsync()
    {
        // Gen 9 optimized minimum fan curve
        var fanTable = new FanTable([0, 0, 1, 2, 3, 5, 7, 10, 15, 20]);
        return Task.FromResult(fanTable);
    }

    /// <summary>
    /// Gen 9 enhanced defaults with vapor chamber support
    /// </summary>
    public override async Task<Dictionary<PowerModeState, GodModeDefaults>> GetDefaultsInOtherPowerModesAsync()
    {
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Getting Gen 9 defaults in other power modes...");
            ...
            if (Log.Instance.IsTraceEnabled)
            {
                Log.Instance.Trace($"Gen 9 defaults in other power modes retrieved:");
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Failed to get Gen 9 defaults in other power modes.", ex);
    ...
    /// <summary>
    /// Gen 9 enhanced default state with AI thermal integration
    /// </summary>
    protected override async Task<GodModePreset> GetDefaultStateAsync()
    {
        ...
        foreach (var c in capabilityData)
        {
            ...
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Creating Gen 9 StepperValue {c.Id}... [idRaw={(int)c.Id:X}, defaultValue={c.DefaultValue}, min={c.Min}, max={c.Max}, step={c.Step}, steps={string.Join(", ", steps)}]");
        ...
        if (Log.Instance.IsTraceEnabled)
            Log.Instance.Trace($"Gen 9 default state retrieved: {preset}");

        return preset;
    }

    /// <summary>
    /// Get Gen 9 sensor data for AI thermal optimization
    /// </summary>
    public async Task<Gen9SensorData> GetGen9SensorDataAsync()
    {
        return await _gen9EcController.ReadSensorDataAsync().ConfigureAwait(false);
    }

    #region Enhanced Gen 9 Helper Methods
    ...
    {
        if (Log.Instance.IsTraceEnabled)
            Log.Instance.Trace($"Reading Gen 9 fan table data...");
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Bad Gen 9 fan table length: {string.Join(", ", fanTableData)}");
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Bad Gen 9 fan table fan speeds length: {string.Join(", ", fanTableData)}");
        ...
        {
            if (Log.Instance.IsTraceEnabled)
                Log.Instance.Trace($"Bad Gen 9 fan table temps length: {string.Join(", ", fanTableData)}");

            return null;
        }

        if (Log.Instance.IsTraceEnabled)
            Log.Instance.Trace($"Gen 9 fan table data: {string.Join(", ", fanTableData)}");
